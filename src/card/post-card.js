const postcardSVG = (title, thumbnail, short_description, tags, user) => {
    ...
    return `
        <svg xmlns="http://www.w3.org/2000/svg" width="440px" height="310px" fill="fffefe">
            <style>
                .header { font: bold 14px Tahoma, Sans-Serif; fill: #343A40; animation: fadeInAnimation 0.8s ease-in-out forwards; }
            ...
            <g data-testid="card-username" transform="translate(${(430 - (430 - (padding * 2))) / 2}, 180)">
                <svg width=${430 - (padding * 2)} height="300">
                    <!-- 유저네임 패딩 적용 -->
                    <text class="header" x="0" y="35" data-testid="header">${user.username + ".Log"}</text>
                </svg>
            </g>
            <g data-testid="card-title" transform="translate(${(430 - (430 - (padding * 2))) / 2}, 212)">
                <svg width=${430 - (padding * 2)} height="300">
                    <!-- 타이틀에 패딩 적용 -->
                    <text class="log-title" x="0" y="35" data-testid="log-title">${title}</text>
                </svg>
            </g>
            <g data-testid="card-body" transform="translate(${(430 - (430 - (padding * 2))) / 2}, 232)">
                <svg width=${430 - (padding * 2)} height="300">
                    <!-- 쇼트 디스크립션에 패딩 적용 -->
